...
bool newBPN12GoAdapter::getInput(int x, int y, const BoardStruct& b, Matrix<float>& input, int colour) const
{
...
*/
	...
	{
		if((i+1)==x)
			input.setValue(0, (count*3)+i, 1);
		else
			input.setValue(0, (count*3)+i, 0);
	}
	...
	{
		if((i+1)==y)
			input.setValue(0, ((count*3)+9)+i, 1);
		else
			input.setValue(0, ((count*3)+9)+i, 0);
	}
...
void newBPN12GoAdapter::setPoint(Matrix<float>& m, int y, int value) const
{
	m.setValue(0, y*3, 0);
