#ifndef NEWBPN2GOADAPTER_H
#define NEWBPN2GOADAPTER_H
...
class newBPN2GoAdapter : public BPNGoAdapterInterface
{
	public:

		// 3 nodes per board point, 18 extra nodes
		newBPN2GoAdapter(const string& filename) : BPNGoAdapterInterface(filename, 3, 18)
		{
			patternWidth = calculatePatternWidth(getBPN());
			patternHeight = calculatePatternHeight(getBPN());;
		};
		virtual ~newBPN2GoAdapter() {};
