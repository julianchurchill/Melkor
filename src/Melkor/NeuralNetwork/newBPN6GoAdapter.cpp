...
bool newBPN6GoAdapter::getInput(int x, int y, const BoardStruct& b, Matrix<float>& input, int colour) const
{
	...
	for(;citer!=points.end();citer++)
	{
		...
		{
			if(b.getLastX()==citer->x && b.getLastY()==citer->y)
				setPointLastMove(input, count++, getActivationValue(citer->type, colour));
			else
...
/*	// find equivalent input values
	for(int i=0;i<pHeight;i++)
	{
		...
		for(int j=0;j<pWidth;j++)
		{
			...
			{
				// is this the point the last move was played on?
				if(b.getLastX()==offsetx && b.getLastY()==offsety)
					setPointLastMove(input, count, getActivationValue(temp.getValue(offsetx, offsety), colour));
				else
...
void newBPN6GoAdapter::setPoint(Matrix<float>& m, int y, int value) const
{
	m.setValue(0, y*4, 0);
...
// functions used to generate input, set 3 unit points etc...
void newBPN6GoAdapter::setPointLastMove(Matrix<float>& m, int y, int value) const
{
	m.setValue(0, y*4, 0);
	m.setValue(0, y*4+1, 0);
	m.setValue(0, y*4+2, 0);
	...
	if(value==ACT_OURCOLOUR)
		...
		m.setValue(0, y*4+1, 2);
	else if(value==ACT_EMPTY)
		m.setValue(0, y*4+2, 2);
	else if(value==ACT_OFFBOARD)
		m.setValue(0, y*4+3, 2);
}