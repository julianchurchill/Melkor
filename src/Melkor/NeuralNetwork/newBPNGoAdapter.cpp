...
bool newBPNGoAdapter::getInput(int x, int y, const BoardStruct& b, Matrix<float>& input, int colour) const
{
	...
	// 3 units per point + 2 units for distance to nearest board edges
...
*/
	// add distance to edge values for last 2 neurons
	// values are between 1 and 10 inclusive 
	// scale between 0 and 2 to keep net from going
	// outside operating range dictated by activation function
	// i.e. for sigmoid is -4, +4 for input value after weight multiplication
...
void newBPNGoAdapter::setPoint(Matrix<float>& m, int y, int value) const
{
	m.setValue(0, y*3, 0);
