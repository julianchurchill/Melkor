...
bool newBPN2GoAdapter::getInput(int x, int y, const BoardStruct& b, Matrix<float>& input, int colour) const
{
...
*/
	...
	for(i=0;i<9;i++)
	{
		if((i+1)==y)
			...
			input.setValues(0, ((count*3)+9)+i, 0);
	}
...
void newBPN2GoAdapter::setPoint(Matrix<float>& m, int y, int value) const
{
	m.setValue(0, y*3, 0);
