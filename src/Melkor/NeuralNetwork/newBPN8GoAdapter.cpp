...
bool newBPN8GoAdapter::getInput(int x, int y, const BoardStruct& b, Matrix<float>& input, int colour) const
{
	...
	for(;citer!=points.end();citer++)
	{
		...
		else if(citer->type==BLACK)
			setPoint(input, count++, ACT_OURCOLOUR);
		else if(citer->type==WHITE)
			setPoint(input, count++, ACT_THEIRCOLOUR);
		else
			setPoint(input, count++, ACT_EMPTY);
	}
	...
	for(int i=0;i<pHeight;i++)
	{
		...
		for(int j=0;j<pWidth;j++)
		{
			...
			{
				// fudge to make this work easily
				int c = temp.getValue(offsetx, offsety);
				if(c==BLACK)
					setPoint(input, count, ACT_OURCOLOUR);
				else if(c==WHITE)
					setPoint(input, count, ACT_THEIRCOLOUR);
...
void newBPN8GoAdapter::setPoint(Matrix<float>& m, int y, int value) const
{
	if(value==ACT_OURCOLOUR)
	{
		m.setValue(0, y*2, 1);
		m.setValue(0, y*2+1, 0);
	}
