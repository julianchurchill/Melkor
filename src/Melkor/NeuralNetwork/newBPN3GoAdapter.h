#ifndef NEWBPN3GOADAPTER_H
#define NEWBPN3GOADAPTER_H
...
class newBPN3GoAdapter : public BPNGoAdapterInterface
{
	public:

		// 4 nodes per board point, 18 extra nodes
		newBPN3GoAdapter(const string& filename) : BPNGoAdapterInterface(filename, 4, 18)
		{
			patternWidth = calculatePatternWidth(getBPN());
			patternHeight = calculatePatternHeight(getBPN());;
		};
		virtual ~newBPN3GoAdapter() {};
