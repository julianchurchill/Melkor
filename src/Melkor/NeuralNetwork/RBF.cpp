...
bool RBF::getAnswer(const Matrix<float>& input)
{
#ifdef _DEBUG
...
//		results1.resize(1, hiddenNodes);
	...
	for(i=0;i<hiddenNodes;i++)
		results1.setValue(0, i, exp(-results1.getValue(0, i)));
...
void RBF::save(string f)
{
	...
	if(!fos)
	{
		message = "RBF: Could not open file for save: ";
		message+=f;
		LogWriter::printerr(message+"\n", typeToString(id));
		return;
	}
...
bool RBF::load(string f, bool quiet /*=false*/)
{
	LogWriter log;
	...
	if(!in)
	{
		message = "RBF: Could not load file: ";
		message+=f;
		LogWriter::printerr(message+"\n");
		return false;
	}
...
void RBF::printInfo(bool printWeights /* = true */) const
{
	LogWriter log;
	char buffer[50];
